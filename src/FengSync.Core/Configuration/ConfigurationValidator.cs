...
public static class ConfigurationValidator
{
    public static IReadOnlyList<string> Validate(ApplicationSettings settings)
    {
        ...
        if (double.IsNaN(settings.MainWindowSidebarWidth) || double.IsInfinity(settings.MainWindowSidebarWidth) || settings.MainWindowSidebarWidth is < 220 or > 380) errors.Add("主窗口侧栏宽度必须介于 220 和 380 之间。");
